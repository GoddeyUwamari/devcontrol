...
-- terminated after MISSING_SCAN_THRESHOLD (3, see resourceReconciliation.service.ts) consecutive
...
COMMENT ON COLUMN aws_resources.missing_scan_count IS
  'Consecutive discovery scans where AWS Resource Explorer successfully ran and did not find this resource. Reset to 0 on any scan that finds it again. Never incremented by a failed/errored scan. Resource is marked status=terminated once this reaches the conservative threshold (3).';
