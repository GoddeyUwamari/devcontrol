...
export class OptimizationController {
  ...
  scan = async (req: Request, res: Response) => {
    try {
      ...
      // first_insight_generated is emitted from the authoritative
      // cost_recommendations flow (see CostRecommendationsController.analyze)
      // -- this table is not read by any customer-facing surface, so it must
      // not double-emit/race the once-per-org guard against the real path.
  ...
  getRecommendations = async (req: Request, res: Response) => {
    try {
      ...
      // first_value_viewed is emitted from the authoritative
      // cost_recommendations flow (see CostRecommendationsController.getAll)
      // -- this endpoint is not called by any customer-facing surface.
